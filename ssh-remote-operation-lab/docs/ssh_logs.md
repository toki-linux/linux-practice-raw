...
| 認証方式 | 公開鍵認証 |
| 確認ログ | `journalctl -u ssh`, `/var/log/auth.log` |
...
このログから、`toki` ユーザーとして公開鍵認証に成功し、SSHセッションが開始されたことが分かる。
...
- `[preauth]` は認証完了前の段階で接続が閉じられたことを示す
...
SSH接続できない時は、接続元のエラー表示だけで判断せず、サーバ側の `journalctl -u ssh` や `/var/log/auth.log` を確認することで、ユーザー名・接続元IP・認証方式・失敗理由を切り分けやすくなる。
