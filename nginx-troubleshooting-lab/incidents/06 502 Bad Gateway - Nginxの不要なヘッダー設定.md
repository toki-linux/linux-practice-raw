...
調査の結果、NginxもPythonアプリサービスも起動しており、ポートも正常にLISTENしていた。
...
Nginx設定を確認すると、Pythonの簡易HTTPサーバには不要な `proxy_http_version 1.1` と `proxy_set_header Connection "upgrade";` が設定されていたため、これを削除して復旧を確認した。
