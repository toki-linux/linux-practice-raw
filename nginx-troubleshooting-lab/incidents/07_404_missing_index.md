...
`403 Forbidden` は、ファイルやディレクトリには到達できているが、権限不足や一覧表示禁止などで表示できない場合に発生する。

一方で、今回の `404 Not Found` は、Python `http.server` が配信対象ディレクトリ内で表示対象のファイルを見つけられなかったために発生した。

```text
403 Forbidden
→ ファイルやディレクトリには到達できたが、表示やアクセスが許可されていない

404 Not Found
→ 探したファイルが存在しない
```

今回のケースでは、NginxはPythonアプリへ正常に中継しており、Python `http.server` 側が `index.html` を見つけられず404を返していた。
