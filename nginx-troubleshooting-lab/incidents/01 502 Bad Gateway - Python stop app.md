...
80番ポートはLISTENしているが、3000番ポートはLISTENしていなかった。
```
### 4. Pythonアプリ単体確認
```bash
...
- error.log の connect() failed (111: Connection refused) while connecting to upstream から、Nginxがupstreamに接続できなかったことを確認できた。
