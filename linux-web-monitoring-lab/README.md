...
この検証では、最初に手動でサービス状態・ポート状態・HTTP応答を確認し、その後Bashスクリプト化した。

さらにcronで定期実行し、`myapp.service` を意図的に停止させた状態で、異常検知と自動復旧ができることを確認した。

検証中、root権限で実行しているにもかかわらず `/tmp/web_stack_check.log` へ書き込めない問題が発生したため、ログ出力先を `/var/log/web_stack_check.log` に変更した。
